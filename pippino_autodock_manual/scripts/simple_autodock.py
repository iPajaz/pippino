...
class DefaultAutoDockConfig(AutoDockConfig):
    ...
    """
    # General Configs
    cam_offset = 0.02           # camera to base link, for edge2edge distance
    front_dock = False
    ...
    # velocity profiles
    linear_vel_range = (-0.08, 0.08)
    angular_vel_range = (-0.06, 0.06)

    max_linear_vel = 0.05         # m/s, for parallel.c and steer
    min_linear_vel = 0.01        # m/s, for lastmile
    max_angular_vel = 0.05       # rad/s
    min_angular_vel = 0.01       # rad/s
    ...
    to_last_mile_tol = 0.15     # transition tolerance from SD to LM
...
class AutoDockStateMachine(AutoDockServer):
    ...
    def __init__(self,
        ...
        super().__init__('auto_dock_node', self.cfg, run_server)
        self.dock_state = DockState.IDLE

    # def init_params(self):
    #     self.get_logger().info("Getting AutoDockServer params from rosparams server")
    ...
    def start(self) -> bool:
        ...
        """
        self.get_logger().info("Start Docking Action Now")
        # self.init_params()
        self.set_aruco_detections(detection_state=True)
        self._loop_rate.sleep()

        current_retry_count = 0
        self.get_logger().info(f"Will attempt with {self.cfg.retry_count} retry")
        ...
        while(True):
            ...
            # If Dock failed
            self.publish_cmd()
            self.get_logger().warn("Failed to Dock attempt")
            ...
            self.get_logger().warn("Attempting retry: "
                          f"{current_retry_count}/{self.cfg.retry_count}")

            if not self.do_retry():
                self.get_logger().warn("Not able to retry")
                self.set_aruco_detections(detection_state=False)
                break
    ...
    def do_retry(self) -> bool:
        ...
        if self.dock_state in [DockState.ACTIVATE_CHARGER,
            ...
            self.get_logger().info("State is not retry-able")
            return False
    ...
    def do_single_side_marker_rotate(self) -> bool:
        ...
        if left_tf is not None:
            self.get_logger().warn(f"Rotate with left marker: {self.cfg.left_marker}")
        ...
        if right_tf is not None:
            self.get_logger().warn(f"Rotate with right marker: {self.cfg.right_marker}")
            yaw = utils.get_2d_pose(right_tf)[2]
            if self.cfg.front_dock:
                yaw = utils.flip_yaw(yaw)
            return self.rotate_with_odom(yaw - math.pi/2)

        self.get_logger().error("Not detecting two side markers, exit state")
        return False
    ...
    def do_predock(self) -> bool:
        ...
        # start predock loop
        _pose_list = []
        self.get_logger().info("Both Markers are detected, running predock loop")
        while not rclpy.ok():
            if self.check_cancel():
                return False
            ...
            if abs(yaw) < self.cfg.stop_yaw_diff:
                self.get_logger().warn(f"Done with yaw correction, tf: {centre_tf}")
                self.publish_cmd()
    ...
    def do_steer_dock(self) -> bool:
        ...
        while not rclpy.ok():
            ...
            if centre_tf is None:
                self.get_logger().warn("Not detecting two sides marker")
                centre_tf_mat = self.get_tf(self.cfg.centre_marker)

                if centre_tf_mat is None:
                    self.get_logger().error("Not able to locate centre marker too")
                    return False
            ...
            if (dis > 0):
                self.get_logger().warn("Transition to lastmile state")
                self.publish_cmd()
                return True
    ...
    def do_last_mile(self) -> bool:
        ...
        while not rclpy.ok():
            ...
            if centre_tf_mat is None:
                self.get_logger().warn("Not detecting centre marker")
                if remaining_dis < self.cfg.max_last_mile_odom:
                    self.get_logger().warn(f"move {remaining_dis}m with odom")
                    return self.move_with_odom(_dir*remaining_dis)
                else:
                    self.get_logger().error("exceeded max_last_mile_odom with "
            ...
            if (remaining_dis <= 0):
                self.get_logger().info(" ~ STOP!! Reach destination! ~")
                self.publish_cmd()
                return True
    ...
    #             res = _trigger()
    #             self.get_logger().info(f"Trigger charger srv, "
    #                           f"success [{res.success}] | msg: {res.message}")
    #             if not res.success:
    #                 self.get_logger().error("trigger charger failed")
    #                 return False
    #         except rospy.ServiceException as e:
    #             self.get_logger().error(f"Charger srv call failed: {e}")
    #             return False
    ...
    #                 if ((rclpy.time.Time() - start).secs >
    #                         self.cfg.check_battery_timeout):
    #                     self.get_logger().error("time out for check battery!")
    #                     return False
    #             self.get_logger().info("Battery is now Charging!")
    #         except rospy.exceptions.ROSException:
    #             self.get_logger().error(f"Failed to get battery state")
    #             return False
...
if __name__ == "__main__":
    ...
    my_config = DefaultAutoDockConfig()
