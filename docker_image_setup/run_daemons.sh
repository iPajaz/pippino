...
if echo "$(uhubctl)" | grep -q "Port 2\: 0203 power 5gbps U0 enable connect \[8087\:0b37 Intel(R) Corporation Intel(R) RealSense(TM) Tracking Camera T265 224622112036\]"; then
	...
	# sleep 2
	# uhubctl -l 2-1.1 -p 1 -a off  # T265
	# sleep 2
	# uhubctl -l 2-1.1 -p 1 -a on  # T265
else
	sudo /sbin/remove_usb_device 1-2.1.1  # reset usb bus
	sleep 2
	uhubctl -l 1-2.1 -p 1 -a off  # T265
	sleep 2
	uhubctl -l 1-2.1 -p 1 -a on  # T265
fi
